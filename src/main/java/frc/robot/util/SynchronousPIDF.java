...
/**
 * Code from Team 254, used under the MIT License. This class implements a PID
 * Control Loop.
 * 
...
public class SynchronousPIDF {
	...
	public double calculate(double input, double dt) {
		...
		if ((m_error * m_P < m_maximumI) && (m_error * m_P > m_minimumI)) {
			// System.out.println("DEBUG: Error is inside izone, integration is
			// occurring-------------------------------------");
			m_totalError += m_error * dt;
		} else {
			// System.out.println("DEBUG: Error is outside izone, no integration
			// occurring------------------------------------");
			m_totalError = 0;
		}
